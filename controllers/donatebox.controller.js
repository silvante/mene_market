                ...
                if (user_doc.status == "admin") {
                    try {
                        ...
                        if (!disabled_box && !active_box) {
                            ...
                            return res.status(201).json({ message: "Quti yaratildi va aktivlashtirildi" })
                        }
                    } catch (err) {
                        console.log(err);
                        res.send(err)
            ...
            jwt.verify(token, jwtSecret, {}, async (err, user_doc) => {
                ...
                if (user_doc.status == "admin") {
                    try {
                        const disabled_box = await Dbox.findOne({ is_active: false })
                        if (disabled_box) {
                            return res.status(404).json({ message: "Quti allachaqon muzlagan" })
                        }
                        const active_box = await Dbox.findOne({ is_active: true })
                        if (active_box) {
                        ...
                        if (!disabled_box && !active_box) {
                            ...
                            return res.status(201).json({ message: "Quti yaratildi va muzlatildi" })
                        }
                    } catch (err) {
                        console.log(err);
                        res.send(err)
