            ...
            jwt.verify(token, jwtSecret, {}, async (err, user_doc) => {
                ...
                if (user_doc.status == "admin") {
                    try {
                        ...
                        res.status(200).json({ message: "status changed to canceled # and user's balance is changed" })
                    } catch (err) {
                        console.log(err);
                        res.send(err)
                    }
            ...
            jwt.verify(token, jwtSecret, {}, async (err, user_doc) => {
                ...
                if (user_doc.status == "admin") {
                    try {
                        ...
                        res.status(200).json({ message: "status changed to success # and user's balance is changed" })
                    } catch (err) {
                        console.log(err);
                        res.send(err)
                    }
